...
OrientationTracker::OrientationTracker(double imuFilterAlphaIn,  bool simulateImuIn) :
  ...
  quaternionMadg{1,0,0,0},
  quaternionEKF{1,0,0,0},
  quaternionMah{1,0,0,0},
  eInt{0.0,0.0,0.0},
  P{}
  {
    initializeEKFCovariance(P, 0.1);
}
...
void OrientationTracker::resetOrientation() {
  ...
  quaternionMadg = Quaternion();
  quaternionEKF = Quaternion();
  quaternionMah = Quaternion();
  initializeEKFCovariance(P, 0.1);
}
